<?php
    ...
    if(isset($_POST['payeer-manual'])){
        if($_POST['trxid'] == ""){
            echo json_encode(["status" => "false", "message" => "Invalid Transaction ID"]);
        }else{
            if($_POST['account_number'] == ""){
                echo json_encode(["status" => "false", "message" => "Invalid Payeer Account Number"]);
            }else{
                $check_transactionid = pp_check_transaction_exits($_POST['trxid']);
                if($check_transactionid['status'] == false){
                    if(pp_set_transaction_byid($payment_id, $plugin_slug, $plugin_info['plugin_name'], $_POST['account_number'], $_POST['trxid'], 'pending')){
            ...
            <div class="payment-form">
                  <ul class="list-unstyled">
                    ...
                    <li class="d-flex">
                      ...
                      <p class="mb-0">
                         Open the <span class="text-primary fw-semibold">Payeer</span> app.
                      </p>
                    </li>
                    ...
                    <li class="d-flex">
                      ...
                      <p class="mb-0">
                        Choose: <span class="text-primary fw-semibold">Transfer</span>
                      </p>
                    </li>
                    ...
                    <li class="d-flex">
                      ...
                      <p class="mb-0">
                        ...
                        <button class="btn btn-primary btn-sm ms-2 px-2 py-1 d-inline-flex align-items-center btn-number-copy" onclick="copyText('<?php echo $settings['account_number']?>', 'btn-number-copy')">
                ...
                <div class="mb-3 account-number-box">
                    <label class="form-label">Payeer Account Number</label>
                    <input type="text" class="form-control account-number" placeholder="Enter payeer account number">
                </div>
    ...
    <script>
        ...
        document.querySelector('.btn-pay').addEventListener('click', function() {
            ...
            if(trxid == ""){
                ...
                if(account_number == ""){
                    document.querySelector(".response").innerHTML = '<div class="alert alert-danger" style="margin-top:10px;margin-bottom:10px"> <i class="fa fa-info-circle me-2"></i> Enter payeer account number</div>';
                }else{
                    ...
                    ({
                        type: "POST",
                        url: "<?php echo pp_get_paymentlink($payment_id)?>?method=payeer-manual",
                        data: { "payeer-manual": "<?php echo $payment_id?>", "trxid": trxid, "account_number": account_number },
