                ...
                <?php
                   ...
                   <?php
                            ...
                            if($_GET['status'] == "ipn"){
                                ...
                                $transactionId = $data['trackId'] ;
                                
                                if ($success && $transactionId) {
                                    $check_transactionid = pp_check_transaction_exits($transactionId);
                                    if($check_transactionid['status'] == false){
                                        if(pp_set_transaction_byid($invoiceId, $plugin_slug, $plugin_info['plugin_name'], "Unknow", $transactionId, 'completed')){
                   ?>
                   ...
                   <?php
                        ...
                        $data = []; 
                        $data['merchant'] = $settings['merchant_api_key'];
                        $data['amount'] = number_format($transaction_amount, 2, '.', '');
                        $data['currency'] = "USD";
                        $data['orderId'] = $payment_id;
                        $data['returnUrl'] = getCurrentUrl() . $separator . "status=return";
                        $data['callbackUrl'] = getCurrentUrl() . $separator . "status=ipn";
                        $data['lifeTime'] = 60;
                        ...
                        if(isset($result['payLink'])){
                            echo '<script>location.href="' . $result['payLink'] . '"</script>';
                        }else{
                   ?>
                                <div class="alert alert-danger" role="alert">
                                   <?php echo $response?>
