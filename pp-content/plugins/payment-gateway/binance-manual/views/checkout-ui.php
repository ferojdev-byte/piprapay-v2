<?php
    ...
    if(isset($_POST['binance-manual'])){
        if($_POST['trxid'] == ""){
            echo json_encode(["status" => "false", "message" => "Invalid Transaction ID"]);
        }else{
            if($_POST['email_id'] == ""){
                echo json_encode(["status" => "false", "message" => "Invalid Email Address or ID"]);
            }else{
                $check_transactionid = pp_check_transaction_exits($_POST['trxid']);
                if($check_transactionid['status'] == false){
                    if(pp_set_transaction_byid($payment_id, $plugin_slug, $plugin_info['plugin_name'], $_POST['email_id'], $_POST['trxid'], 'pending')){
            ...
            <div class="payment-form">
                  <ul class="list-unstyled">
                    ...
                    <li class="d-flex">
                      ...
                      <p class="mb-0">
                         Open the <span class="text-primary fw-semibold">Binance</span> app.
                      </p>
                    </li>
                    ...
                    <li class="d-flex">
                      ...
                      <p class="mb-0">
                        Choose: <span class="text-primary fw-semibold">Send to Binance User</span>
                      </p>
                    </li>
                    ...
                    <li class="d-flex">
                      ...
                      <p class="mb-0">
                        Enter the email or id: 
                        <span class="text-primary fw-semibold bg-primary bg-opacity-10 px-2 py-1 rounded"><?php echo $settings['binance_email_id']?></span>
                        <button class="btn btn-primary btn-sm ms-2 px-2 py-1 d-inline-flex align-items-center btn-number-copy" onclick="copyText('<?php echo $settings['binance_email_id']?>', 'btn-number-copy')">
                ...
                <div class="mb-3 email-id-box">
                    <label class="form-label">Email Address or ID</label>
                    <input type="text" class="form-control email-id" placeholder="Enter email address or id">
                </div>
    ...
    <script>
        ...
        document.querySelector('.btn-pay').addEventListener('click', function() {
            ...
            if(trxid == ""){
                ...
                if(email_id == ""){
                    document.querySelector(".response").innerHTML = '<div class="alert alert-danger" style="margin-top:10px;margin-bottom:10px"> <i class="fa fa-info-circle me-2"></i> Enter email address or id</div>';
                }else{
                    ...
                    ({
                        type: "POST",
                        url: "<?php echo pp_get_paymentlink($payment_id)?>?method=binance-manual",
                        data: { "binance-manual": "<?php echo $payment_id?>", "trxid": trxid, "email_id": email_id },
