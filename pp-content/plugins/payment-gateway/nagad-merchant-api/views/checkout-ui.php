<?php
    ...
    require_once __DIR__ . '/../vendor-nagad-merchant/autoload.php';
    ...
    <div class="payment-container">
        ...
        <div class="payment-body">
            ...
            <div class="payment-form">
                <?php
                    $config = [
                        'NAGAD_APP_ENV' => ($settings['nagad_mode'] ?? 'sandbox') === 'sandbox' ? 'development' : 'production',
                        'NAGAD_APP_LOG' => '1',
                        'NAGAD_APP_ACCOUNT' => $settings['nagad_app_account'], //demo
                        'NAGAD_APP_MERCHANTID' => $settings['nagad_merchant_id'], //demo
                        'NAGAD_APP_MERCHANT_PRIVATE_KEY' => $settings['nagad_private_key'],
                        'NAGAD_APP_MERCHANT_PG_PUBLIC_KEY' => $settings['nagad_public_key'],
                    ...
                    if (strpos($method, '?') !== false) {
                        parse_str(parse_url($method, PHP_URL_QUERY), $method_params);
                        if (isset($method_params['merchant'])) {
                            $merchant = $method_params['merchant'];
                        }
                    }
                        ...
                        if (isset($responseArray['payment_ref_id'], $responseArray['status']) && $responseArray['status'] == "Success") {
                            ...
                            try {
                                ...
                                if (isset($buffer['status']) && $buffer['status'] =='Success'){
                                    $parts = explode("QPL", $buffer['orderId']);
                                    $order_id = $parts[1];
                                    
                                    $check_transactionid = pp_check_transaction_exits($buffer['issuerPaymentRefNo']);
                                    if($check_transactionid['status'] == false){
                                        if(pp_set_transaction_byid($payment_id, $plugin_slug, $plugin_info['plugin_name'], $buffer['clientMobileNo'], $buffer['issuerPaymentRefNo'], 'completed')){
                   ...
                   ?>
                                    <div class="alert alert-danger" role="alert">
                                      Transaction <?php echo $buffer['status']?>
                                    </div>
                   <?php
                                }
                            } catch (Exception $e) {
                   ...
                   ?>
                            <div class="alert alert-danger" role="alert">
                              Transaction <?php echo $responseArray['status']?>
                            </div>
                   <?php
                        }
                    }else{
                        try {
                            ...
                            $combined = $random . 'QPL' . $payment_id;
                            
                            if (strlen($combined) > 20) {
                                $final = substr($combined, strpos($combined, 'QPL') + 1, 20);
                            } else {
                                $final = $combined;
                            }
                            ...
                            $nagad = new Base($config, [
                                ...
                                'invoice' => $final,
                                'merchantCallback' => getCurrentUrl(),
                            ]);
