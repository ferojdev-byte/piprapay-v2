<?php
    ...
    if(isset($_POST['payoneer-manual'])){
        if($_POST['trxid'] == ""){
            echo json_encode(["status" => "false", "message" => "Invalid Transaction ID"]);
        }else{
            if($_POST['email_address'] == ""){
                echo json_encode(["status" => "false", "message" => "Invalid Payoneer email address"]);
            }else{
                $check_transactionid = pp_check_transaction_exits($_POST['trxid']);
                if($check_transactionid['status'] == false){
                    if(pp_set_transaction_byid($payment_id, $plugin_slug, $plugin_info['plugin_name'], $_POST['email_address'], $_POST['trxid'], 'pending')){
                        echo json_encode(["status" => "true", "message" => "Initialize Transaction ID"]);
                    }
                }else{
                   echo json_encode(["status" => "false", "message" => "Transaction ID already exits"]); 
                }
            }
        }
        exit();
            ...
            <div class="payment-form">
                  <ul class="list-unstyled">
                    ...
                    <li class="d-flex">
                      ...
                      <p class="mb-0">
                         Open the <span class="text-primary fw-semibold">Payoneer</span> app or website.
                      </p>
                    </li>
                    ...
                    <li class="d-flex">
                      ...
                      <p class="mb-0">
                        Choose: <span class="text-primary fw-semibold">Pay to recipients payoneer account</span>
                      </p>
                    </li>
                    ...
                    <li class="d-flex">
                      ...
                      <p class="mb-0">
                        ...
                        <span class="text-primary fw-semibold bg-primary bg-opacity-10 px-2 py-1 rounded"><?php echo $settings['account_email']?></span>
                        <button class="btn btn-primary btn-sm ms-2 px-2 py-1 d-inline-flex align-items-center btn-number-copy" onclick="copyText('<?php echo $settings['account_email']?>', 'btn-number-copy')">
                    ...
                    <li class="d-flex">
                      ...
                      <p class="mb-0">
                        Put the <span class="text-primary fw-semibold">Transaction ID</span> in the box below and press <span class="text-primary fw-semibold">Submit</span>
                      </p>
                    </li>
                  </ul>
            
                <div class="mb-3 email-address-box">
                    <label class="form-label">Payoneer Email Address</label>
                    <input type="text" class="form-control email-address" placeholder="Enter payoneer email address">
                </div>
    ...
    <script>
        ...
        document.querySelector('.btn-pay').addEventListener('click', function() {
            ...
            if(trxid == ""){
                ...
                if(email_address == ""){
                    document.querySelector(".response").innerHTML = '<div class="alert alert-danger" style="margin-top:10px;margin-bottom:10px"> <i class="fa fa-info-circle me-2"></i> Enter payoneer email address</div>';
                }else{
                    ...
                    ({
                        type: "POST",
                        url: "<?php echo pp_get_paymentlink($payment_id)?>?method=payoneer-manual",
                        data: { "payoneer-manual": "<?php echo $payment_id?>", "trxid": trxid, "email_address": email_address },
