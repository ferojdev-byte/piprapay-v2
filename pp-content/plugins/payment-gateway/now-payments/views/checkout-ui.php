            ...
            <div class="payment-form">
                <?php
                   ...
                   <?php
                    ...
                    ?>
                                <center>
                                    <img src="https://cdn.pixabay.com/animation/2022/07/29/03/42/03-42-11-849_512.gif" style=" height: 70px; ">
                                    <p style="margin-top:20px;">Your transaction is currently being processed and verified. <br>Please wait a minute.</p>
                                </center>
                    <?php
                            }else{
                                if($_GET['status'] == "ipn"){
                                    ...
                                    if ($payment_status === 'finished' && $purchase_id) {
                                        $check_transactionid = pp_check_transaction_exits($trxID);
                                        if($check_transactionid['status'] == false){
                                            if(pp_set_transaction_byid($payment_id, $plugin_slug, $plugin_info['plugin_name'], $pay_address, $trxID, 'completed')){
                       ?>
                       ...
                       <?php
                        ...
                        $data = [
                            "price_amount" => $transaction_amount,
                            "price_currency" => "usd",
                            "order_id" => $payment_id,
                            "order_description" => "Payment",
                            "ipn_callback_url" => getCurrentUrl() . $separator . "status=ipn",
                            "success_url" => getCurrentUrl() . $separator . "status=success",
                        ...
                        if(isset($response_decode['status']) && $response_decode['status'] == false){
                   ?>
                                <div class="alert alert-danger" role="alert">
                                   <?php echo $response?>
                                </div>
                   <?php
                            ...
                            // Use quotes properly for JavaScript
                            echo '<script>location.href="' . $response_decode['invoice_url'] . '"</script>';
                        }
                    }
                ?>
            </div>
